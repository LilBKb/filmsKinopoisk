...
import type { MovieItem, MoviesResponse } from "../../../interfaces/movies";
...
const Movies = ()=>{
    const {isLoading,hasError,responsePopular,responseBest,responseCartoons,responseSerial,responseFilms} = useMoviesQuery();
    ...
    const dataForCarousel = (data: MovieItem[]): ReactNode[] => {
        ...
        (<AcroolSlideImage key={row.kinopoiskId} imageUrl={row.posterUrlPreview}>
        </AcroolSlideImage>)
        )
    }
    ...
    {carouselArr.map(item=>
        (
            <Stack className={styles.carousel} key={item.title}>
                <Link variant="h4" component={RouterLink} to={item.url} sx={{mt:2,mb:2}} textAlign={'center'}>{item.title}</Link>
                <AcroolCarousel
                data={item.data}
                isEnableNavButton
                slidesPerView={1}
